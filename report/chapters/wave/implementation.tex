...
\end{equation*}
The update method could work in this fashion for any type functions $\rho_i$, but we will only consider  $\rho_m \in C^{\infty}$.
...
only at the boundary. If $\mathcal{T}_h = \{I_n\}_{n=0}^N$ denotes the partition of $\Omega$, where $I_n = (x_n, x_{n+1})$ are the elements defined by the face nodes $0 = x_0,\ldots,x_{N+1}=L$,
then we require 
\begin{equation*}
...
for
\[
    \texttt{a}_n = \frac{\sigma \max(\rho_{m-1}(t), \rho_m(t))}{\min(h_{n-1}, h_n)}.
\]
