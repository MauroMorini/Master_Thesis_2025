...
Lets say we want to send a wave through the waveguide from left to right. We can achieve that by imposing 
...
\begin{equation*}
    u_t(1, \cdot) + \sqrt{c(1,\cdot)} u_x(1,\cdot) = 0 \qquad \text{in } [0, T].
\end{equation*}
...
\begin{equation*}
    -\avg{c(x_{N+1},t) \partial_x u_h(x_{N+1},t)}\jump{v(x_{N+1})} = \avg{\sqrt{c(x_{N+1},t)} \partial_t u_h(x_{N+1},t) } \jump{v(x_{N+1})}.
\end{equation*}

the semi-discrete scheme (\ref{eq:wave_semidiscrete_var_form}) turns into
\begin{equation*}
    (\partial_{t}^2 u_h(t), v)_{L^2(\Omega)} + b_h(u_h,v;t) + \avg{\sqrt{c(x_{N+1},t)} \partial_t u_h(x_{N+1},t) } \jump{v(x_{N+1})} = \ell_h (v;t) 
...
\end{equation*}
where $[\mathbf{R}(t)]_{i,j} = \avg{\sqrt{c(x_{N+1},t)} \Phi_j(x_{N+1},t) } \jump{\Phi_i(x_{N+1})} $ is a matrix where the only non-zero entry is the one at the bottom right corner
