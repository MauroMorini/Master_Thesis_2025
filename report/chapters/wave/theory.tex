...
\begin{lemma}
    \label{lemma:equivalence_norms_on_Vh}
...
\begin{proof}
    ...
    \begin{proofstep}[Introduce Elliptic Projection]
        ...
        \end{equation}
    \end{proofstep}
    \begin{proofstep} [Derive Error Equation]
        ...
        \end{align}
        respectively.
        We can subtract (\ref{eq:proof_wave_l2_convergence_pde_of_cont_solution}) - (\ref{eq:proof_wave_l2_convergence_pde_of_discrete_solution}) and find
        \begin{align*}
    ...
    \end{proofstep}
    \begin{proofstep}[Sum up Error Equations]
        For $j \in \countset{2}{M-2}$ by the definition of $\delta^2$ in (\ref{def:centered_finite_difference_quotient}) we find
        \begin{align*}
            (\delta^2 e^{j+1},v)_{\lo} + (\delta^2 e^{j},v)_{\lo} + (\delta^2 e^{j-1},v)_{\lo} 
            =  \Big(\frac{e^{j+2} - e^{j+1}}{\Delta t^2},v \Big)_{\lo} - \Big(\frac{e^{j-1} - e^{j-2}}{\Delta t^2},v\Big)_{\lo},
        \end{align*}
        therefore summing up all error equations over $j = 1,\ldots,m$ for some $m \in \countset{1}{M-1}$ yields 
        \begin{equation}
            \label{eq:proof_wave_l2_convergence_summed_up_error_equation_with_sums_outside}
            \Big(\frac{e^{m+1} - e^{m}}{\Delta t},v \Big)_{\lo} - \Big(\frac{e^{1} 
            - e^{0}}{\Delta t},v \Big)_{\lo} + \Delta t \sum_{j = 1}^{m} \sipgb(e^j, v) 
            = \Delta t \sum_{j=1}^m (r^j,v)_{\lo},
        \end{equation}
        where we have multiplied both sides with $\Delta t$. 
        Note that the bilinearity of $\sipgb$ has been used here as well as 
        it's time independency. 
        For $m \in \countset{1}{M-1}$ define 
        \begin{align*}
            E^m &:= \Delta t \sum_{j=1}^{m} e^j, && E^0:= 0, \\
            R^m &:= \Delta t \sum_{j=0}^{m} r^j, && r^0:= \frac{e^1 - e^0}{\Delta t^2}.
        \end{align*}
        Applying these definitions and the bilinearity of $\sipgb$ to 
        (\ref{eq:proof_wave_l2_convergence_summed_up_error_equation_with_sums_outside})
        and taking the term $R^0$ to the right hand side yields the accumulated 
        error equation 
        \begin{equation}
            \label{eq:proof_wave_l2_convergence_accumulated_error_equation}
            \Big( \frac{e^{m+1} - e^m}{\Delta t},v \Big)_{\lo} + \sipgb(E^m,v) = (R^m,v)_{\lo} 
            \qquad \forall v \in V_h, m \in \countset{0}{M-1}.
        \end{equation}
        Note that the case $m = 0$ holds by the definition of $R^0, E^0$.
    \end{proofstep}
    \begin{proofstep}[Reform Error Equation into Error Estimate]
        Since (\ref{eq:proof_wave_l2_convergence_accumulated_error_equation})
        holds for all $v \in V_h$, for an arbitrary $j \in \countset{0}{M-1}$ 
        (where we call the variable $j$ in the accumulated error equation   
        (\ref{eq:proof_wave_l2_convergence_accumulated_error_equation}) above
        instead of $m$ as before, because we intend
        to then later again sum over $j=0,\ldots,m-1$) we can choose the test function 
        \begin{equation*}
            v = \frac{e^{j+1} - e^j}{2},
        \end{equation*}
        introduce it into (\ref{eq:proof_wave_l2_convergence_accumulated_error_equation})
        and multiply the equation by $2 \Delta t$. We obtain 
        \begin{equation*}
            \norm{e^{j+1}}_{\lo}^2 - \norm{e^j}_{\lo}^2 + \Delta t \,
            \sipgb(E^j, e^{j+1} + e^j) = \Delta t\, (R^j, e^{j+1} + e^j)_{\lo} 
        \end{equation*}
        for $j \in \countset{0}{M-1}$ arbitrary. Due to the specific choice of  the test function
        $v$ we were able to eliminate the cross terms $ \pm \frac{1}{2 \Delta t }(e^{j+1}, e^{j})_{\lo}$.
        For a fixed $m \in \countset{0}{M}$ we sum 
        over $j = 0,\ldots, m-1$ finding
        \begin{equation*}
            \norm{e^{m}}_{\lo}^2 - \norm{e^0}_{\lo}^2 + 
            \Delta t \sum_{j=0}^{m-1} \sipgb(E^j, e^{j+1} + e^j) 
            = \Delta t \sum_{j=0}^{m-1} (R^j, e^{j+1} + e^j)_{\lo}. 
        \end{equation*}
        We will now focus on the term $ \Delta t \sum_{j=0}^{m} \sipgb(E^j, e^{j+1} + e^j) $
        and rewrite it using the symmetry of the bilinear form.
        Firstly note that 
        \begin{equation}
            \label{eq:proof_wave_l2_summed_up_error_equation_with_chosen_test_funct_pre_estimate}
            E^{j+1} - E^{j-1} = \Delta t \sum_{i = 1}^{j+1} e^i - \Delta t \sum_{i = 1}^{j-1} e^i
            = \Delta t (e^{j+1} + e^{j-1}) \qquad \forall j \in \countset{1}{M-1}.
        \end{equation}
        We can therefore rewrite 
        \begin{align}
            \Delta t \sum_{j = 0}^{m-1}\sipgb(E^j, e^{j+1} + e^j) 
            &= \Delta t \, \sipgb(\underbrace{E^0}_{=0}, e^1 + e^0)
            + \sum_{j = 1}^{m-1}\sipgb(E^j, \Delta t(e^{j+1} + e^j)) \nonumber \\
            &= \sum_{j = 1}^{m-1} \sipgb(E^j, E^{j+1}- E^{j-1})  
            = \sum_{j = 1}^{m-1} \sipgb(E^{j+1}, E^j) - \sipgb(E^j, E^{j-1})\nonumber \\
            \label{eq:proof_wave_l2_convergence_pre_estimate_bh_term_rewrite_1}
            &= \sipgb(E^m, E^{m-1}) - \sipgb(E^1, \underbrace{E^0}_{=0}) 
            = \sipgb(E^m, E^{m-1}).
        \end{align}
        Secondly we find (by again applying the symmetry of $\sipgb$)
        \begin{align}
            \label{eq:proof_wave_l2_convergence_pre_estimate_bh_term_rewrite_2}
            \sipgb\Big(\frac{E^{m} + E^{m-1}}{2}, \frac{E^{m} + E^{m-1}}{2}\Big)
             - \sipgb\Big(\frac{E^{m} - E^{m-1}}{2}, \frac{E^{m} - E^{m-1}}{2}\Big)
             = \sipgb(E^m, E^{m-1}).
        \end{align} 
        Finally after combining (\ref{eq:proof_wave_l2_convergence_pre_estimate_bh_term_rewrite_1}), (\ref{eq:proof_wave_l2_convergence_pre_estimate_bh_term_rewrite_2})
        and introducing them into (\ref{eq:proof_wave_l2_summed_up_error_equation_with_chosen_test_funct_pre_estimate}) after having taken
        the term 
        \[- \norm{e^0}_{\lo}^2 + \Delta t \sum_{j=0}^{m-1} \sipgb(E^j, e^{j+1} + e^j)\]
        to the right hand side we can estimate 
        \begin{align}
            \norm{e^m}_{\lo}^2 &= \norm{e^0}_{\lo}^2 
             \underbrace{ - \sipgb \Big(\frac{E^m + E^{m-1}}{2}, \frac{E^m + E^{m-1}}{2}\Big)}_{\leq 0} \nonumber \\
            &+ \sipgb \Big(\underbrace{\frac{E^m - E^{m-1}}{2}}_{= \Delta t e^m/2}, \frac{E^m - E^{m-1}}{2} \Big)
            + \Delta t \sum_{j = 0}^{m-1} (R^j, e^{j+1} + e^j)_{\lo} \nonumber \\
            &\leq \norm{e^0}_{\lo}^2 + \frac{\Delta t^2}{4} \sipgb(e^m, e^m) 
            + \Delta t \sum_{j = 0}^{m-1}(R^j, e^{j+1} + e^j) \qquad \forall m \in \countset{1}{M},
            \label{eq:proof_wave_l2_convergence_first_estimate_em}
        \end{align}
        where we have used that the bilinear form $\sipgb$ is coercive on $V_h$ 
        and $E^m \in V_h$, therefore implying that 
        \begin{equation*}
            \sipgb \Big( \frac{E^m + E^{m-1}}{2}, \frac{E^m + E^{m-1}}{2} \Big) \geq 0.
        \end{equation*}
    \end{proofstep}
    \begin{proofstep}[Main Estimation Step]
        Using Lemma \ref{lemma:equivalence_norms_on_Vh} we find 
        \begin{equation*}
            \sipgb(e^m, e^m) \leq C_{\text{cont}}C_{\lambda} h^{-2} \norm{e^m}_{\lo}^2.
        \end{equation*}
        and introducing this into (\ref{eq:proof_wave_l2_convergence_first_estimate_em})
        yields
        \begin{equation*}
            \norm{e^m}_{\lo}^2 \leq 
            \norm{e^0}_{\lo}^2 + \frac{C_{\text{cont}}C_{\lambda} \Delta t^2}{4h^2} \norm{e^m}_{\lo}^2
            + \Delta t \sum_{j = 0}^{m-1}(R^j, e^{j+1} + e^j) \qquad \forall m \in \countset{1}{M}. 
        \end{equation*}
        By the assumption 
        \begin{equation*}
            \varepsilon := 1 - \frac{C_{\text{cont}}C_{\lambda} \Delta t^2}{4h^2} > \frac{1}{2}
        \end{equation*}
        we can take the term $\displaystyle \frac{C_{\text{cont}}C_{\lambda} \Delta t^2}{4h^2} \norm{e^m}_{\lo}^2$ to the 
        left hand side of the inequality. Using Cauchy-Schwarz and Young's inequality we estimate
        \begin{align*}
            \varepsilon \norm{e^m}_{\lo}^2 
            &\leq \norm{e^0}_{\lo}^2 
            + \Delta t \sum_{j = 0}^{m-1} (R^j, e^{j+1} + e^j)_{\lo} \\
            &\leq \norm{e^0}_{\lo}^2 
            + \Big( \max_{j = 0,\ldots,M}\norm{e^j}_{\lo}\Big) \Big(\frac{\sqrt{\varepsilon}}{\sqrt{\varepsilon}}\Big)
            2 \Delta t \sum_{j = 0}^{m-1} \norm{R^j}_{\lo} \\
            &\leq \norm{e^0}_{\lo}^2   
            + \max_{j = 0,\ldots,M} \frac{\varepsilon}{2} \norm{e^j}_{\lo}^2
            + \frac{2 \Delta t^2}{\varepsilon} \Big(\sum_{j = 0}^{m-1} \norm{R^j}_{\lo} \Big) \\
            &\leq  \norm{e^0}_{\lo}^2   
            + \max_{j = 0,\ldots,M} \frac{\varepsilon}{2} \norm{e^j}_{\lo}^2
            + \frac{2 \Delta t^2}{\varepsilon} \Big( \sum_{j = 0}^{M-1} \norm{R^j}_{\lo} \Big)^2.
        \end{align*}
        We are now left with a right hand side completely independent of $m \in \countset{1}{M}$.
        This allows us now to take the maximum over $m$ on both sides of the inequality
        (in reality only the left hand side is affected)
        \begin{equation*}
            \max_{j = 0,\ldots,M} \varepsilon \norm{e^j}_{\lo}^2
            \leq  \norm{e^0}_{\lo}^2   
            + \max_{j = 0,\ldots,M} \frac{\varepsilon}{2} \norm{e^j}_{\lo}^2
            + \frac{2 \Delta t^2}{\varepsilon} \Big( \sum_{j = 0}^{M-1} \norm{R^j}_{\lo} \Big)^2,
        \end{equation*}
        and subtracting the term $\displaystyle \max_{j = 0,\ldots,M} \frac{\varepsilon}{2} \norm{e^j}_{\lo}^2$ 
        on both sides yields 
        \begin{equation*}
            \max_{j = 0,\ldots,M} \frac{1}{4} \norm{e^j}_{\lo}^2
            \leq \max_{j = 0,\ldots,M} \frac{\varepsilon}{2} \norm{e^j}_{\lo}^2
            \leq  \norm{e^0}_{\lo}^2   
            + \frac{2 \Delta t^2}{\varepsilon} \Big( \sum_{j = 0}^{M-1} \norm{R^j}_{\lo} \Big)^2.
        \end{equation*}
        We can now estimate 
        \begin{equation*}
            \frac{2 \Delta t^2}{\varepsilon} \Big( \sum_{j = 0}^{M-1} \norm{R^j}_{\lo} \Big)^2
            \leq {8 \Delta t^2} \norm{R^0}_{\lo}^2  
            + {8 \Delta t^2} \Big( \sum_{j = 1}^{M-1} \norm{R^j}_{\lo} \Big)^2
        \end{equation*}
        having used the general inequality $(a+b)^2 \leq 2a^2 + 2b^2$ and $\varepsilon > 1/2$.
        Together we find the estimate 
        \begin{equation}
            \label{eq:proof_wave_l2_convergence_main_estimation_multiple_terms}
            \max_{j = 0,\ldots,M} \norm{e^j}_{\lo}^2 
            \leq 4 \norm{e^0}_{\lo}^2 
            + {8 \Delta t^2} \norm{R^0}_{\lo}^2  
            + {8 \Delta t^2} \Big( \sum_{j = 1}^{M-1} \norm{R^j}_{\lo} \Big)^2
        ...
        \end{equation}
    \end{proofstep}
    \begin{proofstep}[Estimating $T_1$]
        This term itself does not require much work since it follows directly from the definitions
