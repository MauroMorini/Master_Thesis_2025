...
\begin{proof}
    ...
    \begin{proofstep}[Reform Error Equation into Error Estimate]
        ...
        (where we call $m = j$ in the accumulated error equation 
        (\ref{eq:proof_wave_l2_convergence_accumulated_error_equation}) above
        to then later again sum it up over $j$) we can choose the test function 
        \begin{equation*}
            v = \frac{e^{j+1} - e^j}{2},
        \end{equation*}
        ...
        $v$ we were able to eliminate the cross terms $ (e^{j+1}, e^{j})_{\lo}$.
        % TODO: plusminus 
        For a fixed $m \in \countset{0}{M-1}$ we sum up 
        over $j = 0,\ldots, m$ finding
        \begin{equation*}
            \norm{e^{m+1}}_{\lo}^2 - \norm{e^0}_{\lo}^2 + 
            \Delta t \sum_{j=0}^{m} \sipgb(E^j, e^{j+1} + e^j) 
            = \Delta t \sum_{j=0}^{m} (R^j, e^{j+1} + e^j)_{\lo}. 
        \end{equation*}
    \end{proofstep}
\end{proof}
