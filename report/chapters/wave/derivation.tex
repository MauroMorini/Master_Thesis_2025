...
First we pose the problem and recall some analysis on regularity, existence and uniqueness of solutions. 
...
\begin{remark}
    We will be using different notations for derivatives loosely throughout this chapter with the goal of simplifying notation while maintaining clarity depending 
    on the context. In general if we consider a spacial function $v \in H^1$ we denote
    \begin{equation*}
        \frac{\text{d}v}{\text{d}x} = v' = v_x = \partial_x v,
    \end{equation*} 
    and for a temporal function $ w \in C^1$ we denote
    \begin{equation*}
        \frac{\text{d}w}{\text{d}t} = w' = \dot{w} = w_t = \partial_t w .
    \end{equation*} 
    For a space-time function $u \in C^1([0,T];H^1)$ we may use a combination of the above.
\end{remark}
...
\begin{subequations} \label{eq:strong_form_wave}
    \begin{align}
        ...
        u(0, t) = g_{0}(t), \quad u(L, t) &= g_{L}(t) &&\forall t \in [0,T], \\
...
\begin{subequations}
    \label{eq:weak_form_wave_homogeneous_dirichlet_bc}
    \begin{equation}
...
It is well known that the problem (\ref{eq:weak_form_wave_homogeneous_dirichlet_bc}) is well posed (see section 7.2 Theorem 3 in \cite{EvansPDE}). Furthermore we even have 
\begin{equation*}
    u \in C^1([0,T]; L^2(\Omega)) \cap C([0,T];H_0^1(\Omega)),
\end{equation*}
which ensures the validity of the initial conditions (\ref{eq:weak_form_wave_homogeneous_dirichlet_bc}). \\
...
for example $g_0 = g(0, \cdot), g_L = g(L, \cdot)$ for some $g \in C^2([0,T]; H^2(\Omega))$. We can write $u = w + g$, where $w \in C^1([0,T]; L^2(\Omega)) \cap C([0,T];H_0^1(\Omega))$ 
is the unique (weak) solution of 
\begin{align*}
    w_{tt} - (c w_x)_x &= f - g_{tt} + (c g_x)_x   &&\text{in } \Omega \times \left(0,T\right],\\
    w(0,\cdot) = w(L,\cdot) &= 0 && \text{in } [0,T], \\
    w(\cdot, 0) = u_0 - g(\cdot, 0), \quad w_t(\cdot, 0) &= v_0 - g_t(\cdot, 0) && \text{in } \Omega.
\end{align*}
By assuming sufficient regularity of the boundary conditions, initial conditions and the source term $f$ as well as certain compatibility conditions to hold
we can get arbitrarily regular solutions of (\ref{eq:weak_form_wave}). In this chapter we will just assume the solution to satisfy a desired level of
regularity, therefore implying sufficiently regular pde data. More theory on the regularity of solutions of hyperbolic equations can be found in section 7.2 Theorem 6 of \cite{EvansPDE}. \\
Define $H:= \{v \in H^2(\Omega)\, | \, v(0) = g_0, v(L) = g_L\}$ the higher regularity weak problem we consider reads: \\ \\
Find $u \in C^2([0,T]; H)$ such that 
\begin{subequations}
    \label{eq:weak_form_wave}
    \begin{align}
        (u_{tt}(t), v)_{L^2(\Omega)} + a(u(t), v;t) &= (f(t), v)_{L^2(\Omega)} &&\forall v \in C_c^{\infty}(\Omega),t \in [0,T] \\
        u(0,t) = g_0, \quad u(L,t) &= g_L                                   &&\forall t \in [0,T]
    \end{align}
\end{subequations}
...
First we discretize the domain by choosing a partition $\mathcal{T}_h$ of $\Omega$ using the notation introduced in the last chapter. Note at this point that the 
mesh we choose is time-independent, meaning the mesh does not change over time.
...
element $I \in \mathcal{T}_h$ by parts and sum up over all elements. 
After adding the symmetry and penalty terms necessary for the SIPG variational form we find the semi-discrete SIPG
...
\begin{equation}
    ...
    (\partial_{t}^2 u_{h}(t), v)_{L^2(\Omega)} + b_h(u_h,v;t) = \ell(v;t) \qquad \forall v \in V_h, t \in [0,T],
\end{equation}
where 
\begin{align*}
    b_h(u,v;t) & = \sum_{n=0}^N \int_{I_n} c(x,t)u_x(x,t)v_x(x)\, \text{d}x
		-\sum_{n=0}^{N+1} \avg{c(x_n,t)u_x(x_n, t)}\jump{v(x_n)} + \avg{c(x_n,t)v_x(x_n)}\jump{u(x_n,t)} \\
		& + \sum_{n=0}^{N+1} \texttt{a}_n(t)\jump{u(x_n,t)}\jump{v(x_n)},                                   \\
		\ell(v;t)  & = (f(t),v)_{L^2(\Omega)}-g_L(t)c(x_{N+1}^-,t)v_x(x_{N+1}^-) + g_0(t)c(x_0^+,t)v_x(x_0^+) \\
		&+ \texttt{a}_{N+1}(t)g_L(t)v(x_{N+1}^-) + \texttt{a}_0(t) g_0(t) v(x_{0}^+).
\end{align*}
\end{tcolorbox}
...
$ [\mathbf{B}(t)]_{i,j} = b_h(\Phi_j, \Phi_i; t)$ is the \textit{stiffness matrix}, similar to the elliptic case but now with a time-dependent coefficient $c$ and
...
as entries uniquely determining the semi-discrete solution. The double dots over $\mathbf{u}$ denote the second order time-derivative, i.e.\ 
\begin{equation*}
    \ddot{\mathbf{u}} = \frac{\text{d}^2 \mathbf{u}}{\text{d}t^2} .
\end{equation*}
...
an explicit time integration scheme. An explicit scheme, in contrast to an implicit scheme, ensures that we only need to invert the mass matrix and not the stiffness matrix
as well.  \\
First we choose a (stable) stepsize $\Delta t > 0$, we write $t_m = m \cdot \Delta t \in [0, T]$, where we can assume there exists some $M \in \N$ such that $M\Delta t = T$. Denote 
\begin{equation*}
...
\begin{equation}
    \label{def:centered_finite_difference_quotient}
    \ddot{\mathbf{u}}_m \approx \delta^2 \mathbf{u}_m := \frac{ \mathbf{u}_{m+1} - 2 \mathbf{u}_{m} + \mathbf{u}_{m-1}}{\Delta t^2},
\end{equation}
...
\begin{equation}
    ...
    \mathbf{M}\mathbf{u}_{m+1} = \Delta t^2 \mathbf{l}_m + (2 \mathbf{M} - \Delta t^2 \mathbf{B}_m) \mathbf{u}_{m} - \mathbf{u}_{m-1} \qquad m = 1,\ldots,M-1.
\end{equation}
...
displacement to initialize $\mathbf{u}_0$, meaning we can write $ \mathcal{I}_h u_0 = \sum_{n=1}^{\text{dim}(V_h)} \alpha_n(0) \Phi_n $ for some coefficients $
\alpha_n(0) \in \mathbb{R}$ and get
\begin{equation*}
    [\mathbf{u}_0]_i = \alpha_i(0),
\end{equation*}
where $\mathcal{I}_h$ is defined in Definition \ref{def:linear_interp_operator}.
\\
...
\begin{equation*}
    \Delta t < 2 \frac{\lambda_{\min}(\mathbf{M})}{\lambda_{\max} (\mathbf{B}(t)) }  \qquad \forall t \in [0,T],
\end{equation*}
where $\lambda_{\min}, \lambda_{\max}$ denote the minimal and maximal (positive) eigenvalues respectively. The expression is well-defined, since 
$\mathbf{B}(t)$ and $\mathbf{M}$ are SPD for all $t \in [0,T]$. This follows from a slight adaptation of the coercivity proof of the SIPG bilinear form on $V_h$ 
(Theorem \ref{thr:cont_coerc_bilin_form}). For further information on the stability of leapfrog see \cite{groteNumWaveLecture}.
Hence we have derived the fully discrete scheme with the necessary initial conditions and time-stepping in matrix-vector form. \\
Finally we will rewrite the fully-discrete scheme in function form: \\ 

\begin{tcolorbox}[mythmstyle]
    Find the sequence $\{u_h^m\}_{m=0}^M \subset V_h$ such that 
    \begin{subequations}
        \label{eq:wave_fully_discrete_problem}
        \begin{align}
            &(\delta^2 u_h^m, v)_{\lo} + \sipgb (u_h^m, v;t_m) = \ell(v;t_m) &&\forall v \in V_h, m \in \countset{1}{M-1}, \\
            &u_h^0 = \mathcal{I}_h u_0, \quad 
            u_h^1 = u_h^0 + \Delta t \mathcal{I}_h v_0 + \frac{\Delta t^2}{2} \ddot{u}_h^0 \\
            &(\ddot{u}_h^0,v)_{\lo} = \ell(v;0) - \sipgb(u_h^0, v) &&\forall v \in V_h
        \end{align}
    \end{subequations}
\end{tcolorbox}