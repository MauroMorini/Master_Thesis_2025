...
\begin{equation}
    ...
    (\partial_{tt}u_{h}(t), v)_{L^2(\Omega)} + b(u_h,v;t) = \ell(v;t) \qquad \forall v \in V_h, t \in [0,T],
\end{equation}
where 
\begin{align*}
...
\begin{equation}
    ...
    \mathbf{M}\mathbf{u}_{m+1} = \mathbf{l}_m + (2 \mathbf{M} - \Delta t^2 \mathbf{B}_m) \mathbf{u}_{m} - \mathbf{u}_{m-1}.
\end{equation}
