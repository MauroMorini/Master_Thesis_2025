...
\begin{equation}
    ...
    (\partial_{t}^2 u_{h}(t), v)_{L^2(\Omega)} + b_h(u_h,v;t) = \ell_h(v;t) \qquad \forall v \in V_h, t \in [0,T],
\end{equation}
where 
\begin{align*}
    b_h(u,v;t) & = \sum_{n=0}^N \int_{I_n} c(x,t)u_x(x,t)v_x(x)\, \text{d}x
		...
		\ell_h(v;t)  & = (f(t),v)_{L^2(\Omega)}-g_L(t)c(x_{N+1}^-,t)v_x(x_{N+1}^-) + g_0(t)c(x_0^+,t)v_x(x_0^+) \\
...
$ [\mathbf{l}(t)]_i = \ell_h(\Phi_i;t)$ the \textit{load vector}, also time-dependent. Finally $ [\mathbf{u}(t)]_i = \alpha_i(t)$ denotes the solution vector with the time-dependent coefficients
